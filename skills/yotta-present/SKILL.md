...
- 关键指标 → `metrics`（必须是对象列表 `[{label,value,unit?,tone?}]`）；问答 → `qa`（rows 须为 问题/回答 两列，否则判 `table`）；多节长内容 → `report`；数值分布 / 趋势 / 占比 → `chart`
